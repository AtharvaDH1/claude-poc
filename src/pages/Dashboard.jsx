...
import {
  mockMetrics, mockClaims, mockChartData, mockPieData,
  mockTypeData, mockActivity, mockHighPriority
} from '../data/mockData'
    ...
    getDashboardData()
      .then(data => {
        ...
        if (data.recentClaims?.length) {
          // Map backend UPPERCASE columns to frontend format
          setClaims(data.recentClaims.map(c => ({
      ...
      .catch(() => {}) // fallback to mockData on error
  ...
  return (
    <AppLayout pageTitle="Dashboard">
      <div style={{ padding:'24px' }}>
          ...
          {!alertDismissed && mockHighPriority.length > 0 && (
            <div style={{
              ...
              <div style={{ flex:1 }}>
                <span style={{ fontSize:'13px', fontWeight:700, color:'#92400E' }}>
                  {mockHighPriority.length} high-priority claim{mockHighPriority.length>1?'s':''} need{mockHighPriority.length===1?'s':''} immediate attention —&nbsp;
                </span>
                <span style={{ fontSize:'13px', color:'#B45309' }}>
                  {mockHighPriority.map(c=>c.claimant).join(', ')}
                </span>
              </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:'16px', marginBottom:'20px' }}>
            {METRICS.map(m => <MetricCard key={m.key} config={m} value={metrics[m.key] || mockMetrics[m.key]} change={m.change}/>)}
            <ValueCard value={mockMetrics.totalValue} sla={mockMetrics.slaCompliance} overdue={mockMetrics.overdueCount}/>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 300px 200px', gap:'16px', marginBottom:'20px' }}>
            ...
            {card(<>
              ...
              <div style={{ padding:'16px 20px' }}>
                <ResponsiveContainer width="100%" height={190}>
                  <BarChart data={mockChartData} barSize={10} barGap={3}>
            ...
            {/* Donut */}
            {card(<>
              {cardHeader('Status Split',`${mockMetrics.total} total`)}
              <div style={{ padding:'12px 20px 16px' }}>
                <ResponsiveContainer width="100%" height={140}>
                  <PieChart>
                    <Pie data={mockPieData} cx="50%" cy="50%" innerRadius={44} outerRadius={62} paddingAngle={3} dataKey="value">
                      {mockPieData.map((d,i)=><Cell key={i} fill={d.color} stroke="transparent"/>)}
                    </Pie>
                  ...
                  {mockPieData.map(d=>(
                    <div key={d.name} style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                        ...
                        <span style={{ fontSize:'10px', color:T.textSubtle }}>{Math.round(d.value/mockMetrics.total*100)}%</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {mockTypeData.map(d => (
                  <div key={d.name}>
                    ...
                    <div style={{ height:'5px', borderRadius:'99px', background:'#F1F5F9', overflow:'hidden' }}>
                      <div style={{ height:'100%', borderRadius:'99px', background:d.color, width:`${Math.round(d.value/mockMetrics.total*100)}%`, transition:'width 1s ease' }}/>
                    </div>
                    <div style={{ fontSize:'10px', color:T.textSubtle, marginTop:'3px', fontWeight:500 }}>{Math.round(d.value/mockMetrics.total*100)}% of total</div>
                  </div>
                ))}
              </div>
            </>)}
