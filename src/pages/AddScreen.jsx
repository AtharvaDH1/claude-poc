...
import { mockAddCases, mockPool, mockTasks } from '../data/mockData'
...
function CaseSearch({ toast }) {
  ...
  const [results, setResults] = useState(mockAddCases)
...
function CaseAssignment({ toast }) {
  const [unassigned, setUnassigned] = useState(mockAddCases.filter(c=>c.assignedTo==='Unassigned'))
...
function DataUploader({ toast }) {
  ...
  const handleUpload = () => {
    ...
    setUploading(true)
    setTimeout(()=>{ setUploading(false); setUploaded(true); toast('success','Upload Complete',`${file.name} processed successfully.`) }, 1800)
  }
...
function PoolTable({ data, type, toast }) {
    ...
    <AppLayout>
      <div style={{ padding:'24px', fontFamily:'Inter,sans-serif' }}>
        ...
        <div style={{ background:T.card, borderRadius:'12px', border:`1px solid ${T.border}`, boxShadow:'0 1px 3px rgba(0,0,0,0.06)', overflow:'hidden' }}>
          ...
          {activeTab === 'assess-pool'   && <PoolTable data={mockPool} type="Assessment" toast={toast}/>}
          {activeTab === 'approver-pool' && <PoolTable data={mockPool.slice(0,3)} type="Approver" toast={toast}/>}
        </div>
      </div>
    </AppLayout>
  )
