...
import { generateSystemDecision, registerClaim as registerClaimMock } from '../../services/mockServices'
...
export default function DecisionTab({ data, update, policy }) {
  ...
  const handleGenSystemDecision = async () => {
    ...
    try {
      let res
      try { res = await getSystemDecision({ ...data, sumAssured: data.sumAssured||policy?.sumAssured }) }
      catch { res = await generateSystemDecision({ ...data, sumAssured: data.sumAssured||policy?.sumAssured }) }
  ...
  const handleSubmit = async () => {
    ...
    try {
      let res
      try { res = await registerClaimAPI(data) }
      catch { res = await registerClaimMock(data) }
      setClaimNo(res.claimNo)
