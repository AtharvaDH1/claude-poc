...
import { fetchPolicyDetails as fetchPolicyMock, fetchCauseEvents, generateTrapScore, STATES, PLACES_OF_DEATH } from '../../services/mockServices'
...
export default function DemographicsTab({ data, update, policy, setPolicy, onComplete }) {
  ...
  useEffect(() => {
    getCauseEvents().catch(() => fetchCauseEvents()).then(setCauses)
  }, [])
  ...
  const handlePolicySearch = async () => {
    ...
    try {
      ...
      let p
      try { p = await fetchPolicyAPI(data.policyId.trim()) }
      catch { p = await fetchPolicyMock(data.policyId.trim()) }
      setPolicy(p)
  ...
  const handleTrapScore = async () => {
    ...
    try {
      let res
      try { res = await getTrapScore({ gender:data.laGender, sumAssured:data.sumAssured, ageAtDeath:data.laAgeAtDeath, causeOfDeath:data.causeCode, placeOfDeath:data.placeOfDeath, advisorCode:data.advisorCode }) }
      catch { res = await generateTrapScore({ gender:data.laGender, sumAssured:data.sumAssured }) }
      ...
      {sec('intimation','2. Intimation Details','Dates, source and death certificate information',
        <div>
          <Grid cols={3}>
            ...
            <Field label="Place of Death" required><Select value={data.placeOfDeath} onChange={e=>update({placeOfDeath:e.target.value})} options={PLACES_OF_DEATH}/></Field>
