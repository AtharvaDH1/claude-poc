...
export default function ClaimSearch() {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      let found = mockClaims.filter(c => !q || c.id.toLowerCase().includes(q) || c.policy.toLowerCase().includes(q) || c.claimant.toLowerCase().includes(q))
      if (statusFilter !== 'All') found = found.filter(c => c.status === statusFilter)
      if (priorityFilter !== 'All') found = found.filter(c => c.priority === priorityFilter)
      setResults(found)
    }
  }
