...
import { mockPool } from '../data/mockData'
...
export default function PoolSelection() {
  ...
  useEffect(() => {
    api.get('/claims/claimByUsername', { method:'POST' })
      .catch(() => {})
    // Load unassigned claims as pool
    api.post('/claims/claimByUsername', {}).then(res => {
      const items = (res.data || [])
        .filter(c => !c.ASSIGNED_TO || c.ASSIGNED_TO === '')
        .map(c => ({
  ...
  const handleAssign = (item) => {
    ...
    // Call real API to assign
    api.post('/claims/assignClaim', { claimNumber: item.claimId, assignTo: 'current_user', role: 'Assessor' }).catch(() => {})
