  ...
  return (
    <div>
      {claim?.sysRecommendation && (
        <div style={{ marginBottom: '20px', padding: '16px', borderRadius: '10px', background: claim.sysRecommendation === 'Approve' ? '#ECFDF5' : '#FEF2F2', border: `1px solid ${claim.sysRecommendation === 'Approve' ? '#A7F3D0' : '#FECACA'}` }}>
          <div style={{ fontSize: '15px', fontWeight: 900, color: claim.sysRecommendation === 'Approve' ? '#065F46' : '#991B1B' }}>
            System recommendation: {claim.sysRecommendation}
          </div>
          <div style={{ fontSize: '12px', marginTop: '6px', color: WS.textMuted }}>Payout {fmtRs(claim.sysPayableAmount)} · Risk {claim.sysRiskScore}</div>
        </div>
      )}

      {calcAmt && (
        <div style={{ marginBottom: '20px', padding: '14px', background: '#F8FAFC', borderRadius: '10px', border: `1px solid ${WS.border}` }}>
          <div style={{ fontSize: '12px', fontWeight: 700, color: WS.textSubtle, marginBottom: '8px' }}>Calculate amount (Transaction API)</div>
          <pre style={{ fontSize: '11px', margin: 0, overflow: 'auto', maxHeight: '140px' }}>{JSON.stringify(calcAmt, null, 2)}</pre>
        </div>
      )}

      {assessorCanEdit && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '12px', fontWeight: 800, color: WS.primary, textTransform: 'uppercase', marginBottom: '12px' }}>Accessor decision</div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            ...
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <textarea value={verificationRemarks} onChange={(e) => setVerificationRemarks(e.target.value)} rows={3} style={{ width: '100%', marginTop: '4px', padding: '10px', border: `1.5px solid ${WS.border}`, borderRadius: '8px', fontFamily: 'Inter,sans-serif', boxSizing: 'border-box' }} />
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: '8px' }}>
        ...
        <p style={{ fontSize: '12px', color: WS.textMuted, marginBottom: '12px' }}>
          Saves via POST register-claim/update after decision API (update-ass / update-ver). No autosave on other tabs.
        </p>
