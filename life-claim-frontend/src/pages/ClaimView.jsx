...
export default function ClaimView() {
  ...
  useEffect(() => {
    if (!claimId) return
    setLoading(true)
    getClaimByNumber(claimId).then(data => {
    ...
    return data
      ...
      toast('error', 'Load Failed', 'Could not load claim details.')
      navigate('/claim-search')
    }).finally(() => setLoading(false))
  }, [claimId, navigate, toast])

  const handleQCSave = async (status, remarks) => {
    setSaving(true)
    try {
      await changeClaimStatus(claimId, status, remarks)
    ...
    setShowQC(false)
      ...
      toast('error', 'Update Failed', 'Could not update claim status.')
    } finally {
      setSaving(false)
    }
  }
